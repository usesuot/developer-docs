...
final class IssueCteService
{
    ...
    public function issue(string $operationId): IssueCteResult
    {
        try {
            $result = $this->cte->issue()
        ...
        if ($result->isAuthorized()) {
            // Host: persistir accessKey, protocolNumber, submittedDocumentXml, responseXml
        }
        ...
        if ($result->isRejected()) {
            ...
            $this->logger->warning('CT-e rejeitado', [
                'code'        => $failure?->code,
                'operationId' => $operationId,
                'retryMode'   => $result->retryPolicy()?->mode->value,
            ]);
            // Host: AfterCorrection / QueryBeforeRetry / Never conforme RetryMode
        }
